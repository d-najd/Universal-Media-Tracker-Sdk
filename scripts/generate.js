...
const excludePatterns = [
    '**/*.test.ts', '**/*.spec.ts', '**/*.d.ts',     // exclude tests & declaration files
...
function hasNamedExport(content) {
    const namedExportRegex = /\bexport\s+(?!default\b)(?:{|\*|const|let|var|function|class|interface|type|enum)\b/;
    return namedExportRegex.test(content);
}

async function main() {
    // Find all TypeScript source files
    const files = await glob(`${srcDir}/**/*.{ts,tsx}`, { ignore: excludePatterns });
    ...
    for (const file of files) {
        ...
        if (hasDefault) {
            const exportName = useFileNameAsExportName ? toIdentifier(relativePath) : 'default';
